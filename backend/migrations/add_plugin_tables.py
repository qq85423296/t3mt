    ...
    try:
        with get_db() as conn:
            ...
            if not get_table_exists(cursor, 'plugin_info'):
                ...
                ''')
                logger.info("  ✅ 已创建 plugin_info 表索引")
            else:
                logger.info("  ⏭️ plugin_info 表已存在，跳过创建")
            ...
            if not get_table_exists(cursor, 'task_plugin_relation'):
                ...
                ''')
                logger.info("  ✅ 已创建 task_plugin_relation 表")
                ...
                ''')
                logger.info("  ✅ 已创建 task_plugin_relation 表索引")
            else:
                logger.info("  ⏭️ task_plugin_relation 表已存在，跳过创建")
            ...
            if not get_table_exists(cursor, 'plugin_exec_log'):
                ...
                ''')
                logger.info("  ✅ 已创建 plugin_exec_log 表")
                ...
                ''')
                logger.info("  ✅ 已创建 plugin_exec_log 表索引")
            else:
                logger.info("  ⏭️ plugin_exec_log 表已存在，跳过创建")
            
            conn.commit()
            logger.info("✅ 插件系统表迁移完成")
...
def downgrade():
    ...
    try:
        with get_db() as conn:
            ...
            logger.info("  ✅ 已删除 plugin_exec_log 表")
            
            cursor.execute("DROP TABLE IF EXISTS task_plugin_relation")
            logger.info("  ✅ 已删除 task_plugin_relation 表")
            
            cursor.execute("DROP TABLE IF EXISTS plugin_info")
            logger.info("  ✅ 已删除 plugin_info 表")
            
            conn.commit()
            logger.info("✅ 插件系统表回滚完成")
